        ...
        try {
            ...
            return response()->json([
                'data' => [
                     ...
                     * @return (null|scalar)[]
                     *
                     * @psalm-return array{id: int, price: float, store_id: int, store: string, store_url: null|string, is_available: bool}
                     */
    ...
    public function search(Request $request): JsonResponse
    {
        try {
            ...
            foreach (['q', 'query', 'name'] as $param) {
                $headerValues = $request->headers->get($param, null, false);
            ...
            $candidate = $request->query('q')
                ?? $request->input('q')
                ?? $request->header('q');
            $candidate = $candidate ?? ($request->query('query')
                ?? $request->input('query')
                ?? $request->header('query'));
            $candidate = $candidate ?? ($request->query('name')
                ?? $request->input('name')
                ?? $request->header('name'));
